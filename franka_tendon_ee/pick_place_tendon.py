...
def grip_lift_ctrl(data, model, hand_id, cube_id, target_pos, extra_pull_mm,
    ...
    def iter_fn(i):
        ...
        # Rate-limited IK on separate data (never touches sim state)
        if i % ik_interval == 0:
            desired_pos = start_pos + (target_pos - start_pos) * t
            ik_data.qpos[:] = data.qpos[:]
            mujoco.mj_forward(model, ik_data)
            q_solve, _ = solve_ik(ik_data, model, hand_id,
                                  desired_pos, target_rot,
                                  max_steps=200, tol=0.002)
            alpha = 0.15
            current_target += alpha * (q_solve - current_target)
        ...
        # Log cube trajectory every 10 steps
        if i % 10 == 0:
            rel = cube_in_hand_frame(data, hand_id, cube_id)
            cur_r = R.from_matrix(data.xmat[hand_id].reshape(3, 3))
            ori_err = (cur_r * target_rot.inv()).as_rotvec()
            ori_deg = np.rad2deg(np.linalg.norm(ori_err))
            CUBE_TRAJECTORY.append((i, rel[0], rel[1], rel[2], ori_deg))
...
def run():
    ...
    desc_tgt = np.array([0.50, 0.0, 0.092])
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for name, action in phases:
            ...
            if action_type == "grip_adaptive":
                ...
                label = f"{target_pos[2]:.3f} z + {extra_pull:.0f}mm pull, {steps} steps, ik={ik_int}"
                print(f"cartesian lift {label}")
                grip_lift_ctrl(data, model, hand_id, cube_id, target_pos, extra_pull, steps,
                ...
                for f in FINGERS:
                    tid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_TENDON, f"{f}_flexor")
                    LOCKED_LENSPRING[f] = float(model.tendon_lengthspring[tid, 0])
                    print(f"  {f}_flexor springlen frozen at {LOCKED_LENSPRING[f]*1000:.2f}mm")
                set_joint_stiffness(model, 100.0)
                print(f"  finger stiffness set to 100 N·m/rad (locked)")
            ...
            elif action_type == "pin_cube":
                cube_pos = data.xpos[cube_id].copy()
                print(f"pinning cube at ({cube_pos[0]:.3f}, {cube_pos[1]:.3f}, {cube_pos[2]:.3f})")
                pin_cube_at(data, cube_pos)
        ...
        if CUBE_TRAJECTORY:
            print(f"\n=== Cube Trajectory (hand frame, every 10 steps, {len(CUBE_TRAJECTORY)} samples) ===")
            print(f"  {'step':>5s}  {'rel_x':>8s}  {'rel_y':>8s}  {'rel_z':>8s}  {'ori_err°':>8s}")
            for step, rx, ry, rz, oerr in CUBE_TRAJECTORY:
                print(f"  {step:5d}  {rx:8.4f}  {ry:8.4f}  {rz:8.4f}  {oerr:8.3f}")
